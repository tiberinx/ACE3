...
[
    QGVAR(useChargeSystem), "CHECKBOX",
    [LSTRING(useChargeSystem_DisplayName), LSTRING(useChargeSystem_Description)],
    LSTRING(category_DisplayName),
    false, // default value
    true, // isGlobal
    {[QGVAR(useChargeSystem), _this] call EFUNC(common,cbaSettings_settingChanged)},
    true // Needs mission restart
] call CBA_settings_fnc_init;
