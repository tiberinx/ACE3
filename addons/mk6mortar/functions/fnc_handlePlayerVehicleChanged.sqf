...
[{
    ...
    if ((vehicle ACE_player) != _mortarVeh) then {
        ...
        private _chargeText = if (_currentFireMode != "SingleFull") then {
            format ["<t size='0.8'>%1: %2 <img image='%3'/></t>", (localize LSTRING(rangetable_charge)), _currentChargeMode, QPATHTOF(UI\ui_charges.paa)];
        } else {
            ""
        };
